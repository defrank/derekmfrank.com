...
class UserImage(models.Model):
    ...
    def get_upload_dir(instance, filename):
        return 'accounts/%s/img/%s' % (instance.user.user.username, filename)
    ...
    def create(cls, user, title, image, default):
        if default:
            old = UserImage.objects.get(user=user, default=True)
            old = models.F(False)
            old.save()
    ...
    def save(self, *args, **kwargs):
        """
        if self.default:
            old = UserImage.objects.get(user=self.user, default=True)
...
class UserDocument(models.Model):
    ...
    def get_upload_dir(instance, filename):
        return 'accounts/%s/doc/%s' % (instance.user.user.username, filename)
